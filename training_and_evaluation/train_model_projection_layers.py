...
checkpoint_path = path_to_weights_pla + 'checkpoint_mod.pt'
...
clip_train_data = ClipDataset(dataset=ds['train'],preprocess=preprocess,tokenize=clip.tokenize)
train_dataloader = DataLoader(clip_train_data,batch_size=batch_size,shuffle=True)

clip_val_data = ClipDataset(dataset=ds['test'],preprocess=preprocess,tokenize=clip.tokenize)
...
for epoch in range(num_epochs):
    ...
    if val_epoch_loss < best_loss:
        ...
        torch.save(model, path_to_weights_pla + "ViT-L-14.pt")
...
with open(os.path.join(path_to_weights_pla,"training_time.txt"),"a+") as f:
    f.write(f"Training time : {str(end_time)}")

# save the loss function plot 
plot_losses(train_losses, val_losses, path_to_plot_pla + "train_val_loss_plot.png")  
#------------------------#
# train model for now !

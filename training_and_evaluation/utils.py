...
class DatasetAugmentGenerator:
    ...
    def image_augment(self,data,target_height,target_width):
        ...
        for i,path in enumerate(data2['image']):
            ...
            transforms = T.Compose([
                ...
                #v2.ConvertImageDtype(torch.float32),  # Normalize expects float input 
    ...
    def caption_augment(self):
        self.data3 = self.data['issue2'].map(lambda x : self.caption_augment_helper(x)).reset_index(drop=True)
    ...
    def generate_df(self):
        ...
        self.final_df.drop(columns=['Unnamed: 0'])
    ...
    def final_fix_dataframe_naming(self):
        ...
        self.new_df_mapped.drop(columns=['Unnamed: 0'],inplace=True)
...
class DuplicateCleaner:
    ...
    def __call__(self):
        for i in range(len(self.data['issue2'])):
            countDuplicate(self.v2,self.data.loc[i,'issue2'],i)
        ...
        data_clean.drop(columns=['Unnamed: 0','Unnamed: 0.2','Unnamed: 0.1'],inplace=True)
